...
describe('Novel Mode: STOP / Abort Lifecycle & Per-Tab Isolation Tests', () => {
    ...
    describe('Test 1: cancel(Tab A) 標記分頁為已中止', () => {
        it('cancel 成功將指定 tabId 標記為 cancelled', () => {
    ...
    describe('Test 3: begin(Tab A, newSessionId) 清除 Tab A 的中止標記', () => {
        it('新 session 啟動時 begin 可正確重置該分頁的中止狀態', () => {
            ...
            registry.begin(101, 'sess-101-new');
            assert.equal(registry.isCancelled(101), false);
            assert.equal(registry.getActiveSessionId(101), 'sess-101-new');
        });
    });

    describe('Test 4: begin(Tab A, newSessionId) 絕不清除 Tab B 的中止標記', () => {
    ...
    describe('Test 5: Queue Pruning (修剪佇列中已中止分頁的所有未執行任務)', () => {
        it('從混合佇列 [A1, B1, A2, B2] 中中止 A 時，精確保留 [B1, B2]', () => {
            const initialQueue = [
                { tabId: 101, sessionId: 'sess-101', batchIndex: 0, texts: ['A1'] },
                { tabId: 202, sessionId: 'sess-202', batchIndex: 0, texts: ['B1'] },
                { tabId: 101, sessionId: 'sess-101', batchIndex: 1, texts: ['A2'] },
                { tabId: 202, sessionId: 'sess-202', batchIndex: 1, texts: ['B2'] }
            ];
    ...
    describe('Test 6: Pre-request Policy (API 呼叫前確認中止)', () => {
        it('模擬 Background 佇列循環：在 API 呼叫前偵測到已中止時直接跳過，API 呼叫 0 次', () => {
            ...
            let apiCallCount = 0;
            const task = { tabId: 101, sessionId: 'sess-101', batchIndex: 0, texts: ['hello'] };
    ...
    describe('Test 7: Post-request Policy (API 呼叫中途中止後捨棄結果)', () => {
        it('模擬 API 傳輸期間使用者按 STOP：API 返回後偵測到已中止，捨棄譯文與寫入', () => {
            ...
            if (registry.isCurrentSession(task.tabId, task.sessionId)) {
                novelResultsWritten++;
                injectMessageSent++;
            }
    ...
    describe('Test 8: Cancelled Task 絕不發送 Success Inject', () => {
        it('已被中止之分頁任務絕不發送 injectNovelBatchResult 訊息給前台', () => {
            ...
            const task = { tabId: 101, sessionId: 'sess-101', batchIndex: 0 };
    ...
    describe('Test 9: Cancelled Task 遭遇 API 錯誤時絕不發送 Failure Inject', () => {
    ...
    describe('Test 10: Cancelled Task 絕不觸發 Next Batch 拉取', () => {
    ...
    describe('Test 11: Manga 全域 isStopping 絕不影響小說任務 (跨模式隔離)', () => {
        it('即使漫畫模式正在 STOP (isStopping = true)，活躍的小說任務 (shouldProcessNovelTask) 依然允許執行', () => {
            const registry = createNovelCancellationRegistry();
            registry.begin(101, 'sess-101');

            // 小說 Tab 101 未中止
            const novelTask = { tabId: 101, sessionId: 'sess-101', batchIndex: 0, texts: ['小說段落'] };
    ...
    describe('Test 12: 多分頁小說佇列交錯時，Tab A 中止僅跳過 A，Tab B 依然正常執行', () => {
        it('模擬佇列循環處理 [TaskA1, TaskB1, TaskA2, TaskB2]：A 中止時僅 A 被 skip，B 順利執行完成且佇列不 break', () => {
            ...
            const queue = [
                { tabId: 101, sessionId: 'sess-101', batchIndex: 0, id: 'A1' },
                { tabId: 202, sessionId: 'sess-202', batchIndex: 0, id: 'B1' },
                { tabId: 101, sessionId: 'sess-101', batchIndex: 1, id: 'A2' },
                { tabId: 202, sessionId: 'sess-202', batchIndex: 1, id: 'B2' }
            ];
