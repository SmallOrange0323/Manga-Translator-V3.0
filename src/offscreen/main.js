...
async function getTesseractWorker() {
    ...
    initPromise = (async () => {
        try {
            log.info('OffscreenAI', '正在初始化本地端日文漫畫 OCR Worker (首次使用將自動下載語言模型)...');
            ...
            log.warn('OffscreenAI', `Tesseract Worker 初始化失敗: ${err.message}，嘗試退回基礎模式...`);
            try {
                const fallbackWorker = await createWorker('jpn');
